...
export class PeopleByGenderChartComponent implements OnInit {
  ...
  static readonly defaultColor = { color: 'bg-light', textColor: 'text-dark', title: 'нет данных' };

  readonly colorsByGrade: Map<Gender, {color: string, textColor: string, title: string}> = new Map([
    [Gender.Female, { color: 'bg-primary', textColor: 'text-white', title: 'Женщины' }],
    [Gender.Male, { color: 'bg-success', textColor: 'text-white', title: 'Мужчины' }],
    [Gender.PreferNotToSay, { color: 'bg-info', textColor: 'text-dark', title: 'Предпочли не указывать' }],
    [Gender.Other, { color: 'bg-warning', textColor: 'text-dark', title: 'Другое' }],
    [Gender.Undefined, PeopleByGenderChartComponent.defaultColor],
  ]);

  colorByGradeLegend: Array<{gender: Gender, color: string, textColor: string, title: string}> = [];
  ...
  private initCharts(): void {
    ...
    const localSararies = this.salaries.filter((item) => item.company === CompanyType.Local);
    const remoteSararies = this.salaries.filter((item) => item.company === CompanyType.Remote);
  ...
  private prepareData(salaries: Array<UserSalary>, showPercents: boolean): Array<ProgressBarData> {
    const totalCount = salaries.length;

    const salariesGroupedByGender: Array<{gender: Gender | null, salaries: Array<UserSalary>}> = [];
    salaries
      .forEach((item) => {

        const existingGroup = salariesGroupedByGender.find((x) => x.gender == item.gender);
    ...
    const result = salariesGroupedByGender
      ...
      .map((item, index) => {
        const color = this.colorsByGrade.get(item.gender!) ?? PeopleByGenderChartComponent.defaultColor;
    ...
    const noGradeData = salariesGroupedByGender.find((item) => item.gender === Gender.Undefined || item.gender == null);
    if (noGradeData != null) {
      const color = this.colorsByGrade.get(Gender.Undefined) ?? PeopleByGenderChartComponent.defaultColor;
