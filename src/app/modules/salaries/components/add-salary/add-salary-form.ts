...
export class AddSalaryForm extends FormGroup {
  ...
  constructor(
    ...
    super({
      ...
      quarter: new FormControl(salarytoBeEdited?.quarter ?? currentQuarter, [
        Validators.pattern(AddSalaryForm.digitsPattern),
        Validators.min(1),
        Validators.max(4),
        Validators.required,
      ]),
      year: new FormControl(salarytoBeEdited?.year ?? now.getFullYear(), [
        Validators.pattern(AddSalaryForm.digitsPattern),
        Validators.min(2000),
        Validators.max(2100),
        Validators.required,
      ]),
      currency: new FormControl(salarytoBeEdited?.currency ?? Currency.KZT, [
        Validators.required,
      ]),
      company: new FormControl(salarytoBeEdited?.company ?? null, [
        Validators.required,
      ]),
      grade: new FormControl(salarytoBeEdited?.grade ?? null, [
        Validators.required,
      ]),
      profession: new FormControl(salarytoBeEdited?.professionId ?? null, [
        Validators.required,
      ]),
      ...
      workIndustryId: new FormControl(
        salarytoBeEdited?.workIndustryId ?? null,
